...
import com.semantyca.core.llm.OpenAiTextClient;
...
@ApplicationScoped
public class MasterPromptTranslateOpenAiService {
    ...
    public Uni<AgentRespDTO> translateMasterPrompt(MasterPromptTranslateReqDTO dto) {
        try {
            ...
            return openAiTextClient.createTextMessage(
                            config.getOpenAiApiKey().orElseThrow(() -> new IllegalStateException("datanest.openai.api-key is not configured")),
                            config.getOpenAiModel(),
                            config.getOpenAiMasterPromptTranslateMaxTokens(),
                            system,
                            userMessage)
                    .map(r -> {
