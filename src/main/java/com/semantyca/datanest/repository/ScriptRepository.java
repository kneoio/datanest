...
public class ScriptRepository extends AsyncRepository {
    ...
    public Uni<List<Script>> getAllShared(int limit, int offset, final IUser user, final ScriptFilter filter) {
        String sql = """
                    ...
                    FROM %s t
                    WHERE EXISTS (
                        SELECT 1 FROM %s rls WHERE rls.entity_id = t.id AND (rls.reader = %s OR rls.reader = 1)
                    ) AND t.archived = 0
                """.formatted(entityData.getTableName(), entityData.getRlsName(), user.getId());
    ...
    public Uni<Integer> getAllSharedCount(IUser user, ScriptFilter filter) {
        String sql = "SELECT COUNT(*) FROM " + entityData.getTableName() + " t " +
                "WHERE EXISTS (SELECT 1 FROM " + entityData.getRlsName() +
                " rls WHERE rls.entity_id = t.id AND (rls.reader = " + user.getId() + " OR rls.reader = 1)) AND t.archived = 0";
    ...
    public Uni<Script> insert(Script script, List<RlsActionDTO> rlsActions, IUser user) {
        return Uni.createFrom().deferred(() -> {
            try {
                String sql = "INSERT INTO " + entityData.getTableName() +
                        " (author, reg_date, last_mod_user, last_mod_date, name, slug_name, default_profile_id, description, language_tag, timing_mode, required_variables, custom) " +
    ...
    public Uni<Script> update(UUID id, Script script, IUser user) {
        return update(id, script, List.of(), user);
    }

    public Uni<Script> update(UUID id, Script script, List<RlsActionDTO> rlsActions, IUser user) {
        return Uni.createFrom().deferred(() -> {
            try {
                JsonArray requiredVarsJson = null;
                if (script.getRequiredVariables() != null && !script.getRequiredVariables().isEmpty()) {
    ...
    public Uni<List<BrandScript>> findForBrand(UUID brandId, final int limit, final int offset,
        ...
        String sql = "SELECT t.*, bs.rank, bs.active, bs.user_variables, " +
                ...
                "WHERE bs.brand_id = $1 AND EXISTS (" +
                "SELECT 1 FROM " + entityData.getRlsName() + " rls WHERE rls.entity_id = t.id AND (rls.reader = $2 OR rls.reader = 1))";
    ...
    public Uni<List<BrandScript>> findForBrandByName(String brandName, final int limit, final int offset, IUser user) {
        String sql = "SELECT t.*, " +
                ...
                "FROM " + entityData.getTableName() + " t " +
                "WHERE EXISTS (" +
                "SELECT 1 FROM " + entityData.getRlsName() + " rls WHERE rls.entity_id = t.id AND (rls.reader = " + user.getId() + " OR rls.reader = 1)" +
                ") AND t.archived = 0" +
