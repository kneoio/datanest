...
public class SoundFragmentController extends AbstractSecuredController<SoundFragment, SoundFragmentDTO> {
    ...
    public void setupRoutes(Router router) {
        ...
        router.route(HttpMethod.GET, path + "/unassigned-brands").handler(this::getUnassignedBrands); //it is archived from regular POV
    ...
    private void getUnassignedBrands(RoutingContext rc) {
        int page = Integer.parseInt(rc.request().getParam("page", "1"));
        int size = Integer.parseInt(rc.request().getParam("size", "10"));
        SoundFragmentFilter filter = parseFilterDTOForUnassigned(rc);
    ...
    private SoundFragmentFilter parseFilterDTOForUnassigned(RoutingContext rc) {
        return parseFilterDTO(rc, null);
    }
